...
import register_input_validate from "../../utils/register_input_validate.jsx"; // for validation - using Regex  -
...
const SignUp = ({setShowCmp}) => {
  ...
  const HandleSubmit = async (e) => {
    e.preventDefault();
    let result = register_input_validate(
      data.fname,
      data.email,
      data.number,
      data.gender,
    ...
    try {
      ...
      // user.uid
      //   ? navigate("/").then(() => alert("login -SuccessFull"))
      //   : alert("Login Failed");
      ...
      console.log("Catch Error Handler ", error);
    }
  };
  ...
  return (
    <form
      className="w-[450px]  rounded-xl bg-white flex flex-col  gap-5 min-h-[400px] p-5"
    ...
    >
      ...
      <div className="flex flex-col gap-1 ">
        ...
        <input
          ...
          value={data.fname}
          className=" px-3 py-2  rounded-xl bg-blue-100"
          id="fname"
          name="fname"
      ...
      <div className="flex flex-col gap-2 position-relative">
        ...
        <button
          type="button"
          className="px-3 py-2  rounded-xl bg-blue-100 position-absolute"
          onClick={HandleShow}
        >
          {show ? "Hide" : "show"}
        </button>
